# MS Billing Software — Project Report
...
MS Billing Software is an **offline-first desktop application** for Indian small businesses. It provides GST-compliant invoicing, customer/supplier management, stock tracking, and basic accounting — all stored locally with SQLite. Runs as a standalone `.exe` on Windows.
...
| App Icon | Custom `logo.ico` (high quality Windows taskbar rendering) |
...
SQLite DB stored at: `C:\Users\<you>\AppData\Roaming\ms-billing-software\ms_billing.db`
...
2. Run the build command: 
   ...
   npm run build
   ```
3. Wait for `electron-builder` to finish. It will create a new setup file in the `dist` folder, such as `MS Billing Software Setup 1.1.0.exe`.
...
- The installer **overwrites the application code** located in `AppData\Local\Programs\ms-billing-software`.
- The user's actual business data (the database file) is stored securely in `AppData\Roaming\ms-billing-software`. 
...
| **Development Data Segregation** | Separated dev and production database paths (`ms-billing-software-dev`) in `main.js` to prevent dummy data colliding with real installation data. |
...
| Invoice numbers reset | Never delete `AppData\Roaming\ms-billing-software` folder |
...
*MS Billing Software v1.2.0 — Last updated March 2026*
...
*MS Billing Software v1.2.1-security-ui � Last updated March 20, 2026*
...
- Updated setup generated at:
  - `dist/MS Billing Software Setup 1.0.0.exe`
